...
class IPToSATSetup(Screen, ConfigListScreen):
	...
	def __init__(self, session):
		...
		if BoxInfo.getItem("distro") in ("norhap", "openspa") and exists(PLAYLIST_PATH):
			...
			if exists(ENIGMA2_PATH_LISTS + "iptosatjsoncard") and exists(str(OSCAM_SERVER)):
				self["key_blue"].setText(language.get(lang, "195"))  # noqa: F821
			elif exists(ENIGMA2_PATH_LISTS + "iptosatjsonall") and exists(str(OSCAM_SERVER)):
			...
			try:
				...
				if not fileContains(CONFIG_PATH_CATEGORIES, "null") and fileContains(CONFIG_PATH_CATEGORIES, ":") and m3u:
					...
					with open(CATEGORIES_TIMER_OK, "w") as fw:
						...
						fw.write(now)
					eConsoleAppContainer().execute('python3 ' + str(BUILDBOUQUETS_SOURCE) + " ; mv " + str(BOUQUET_IPTV_NORHAP) + ".del" + " " + str(BOUQUET_IPTV_NORHAP) + " ; wget -qO - http://127.0.0.1/web/servicelistreload?mode=2 ; rm -f " + str(self.m3ufile) + " ; mv " + str(BUILDBOUQUETS_SOURCE) + " " + str(BUILDBOUQUETS_FILE) + " ; echo 1 > /proc/sys/vm/drop_caches ; echo 2 > /proc/sys/vm/drop_caches ; echo 3 > /proc/sys/vm/drop_caches")
					if self.storage:
...
class AssignService(ChannelSelectionBase):
	...
	def doinstallChannelsList(self, answer):
		try:
			...
			tuxboxfiles = ""
			backupfilestuxbox = ""
			camfolders = FILES_TUXBOX + '/config/'
			wlan = False
			if answer:
				...
				for filestuxbox in [x for x in listdir(self.backupdirectory) if ".xml" in x]:
					...
					if backupfilestuxbox:
						for fileschannelslist in [x for x in listdir(FILES_TUXBOX) if ".xml" in x and "timezone.xml" not in x]:
							...
							if tuxboxfiles:
								remove(tuxboxfiles)
				dumped = ' ; init 3 ; mount -a ; mv -f ' + ENIGMA2_PATH_LISTS + '*oscam*' + " " + camfolders + ' ; mv -f ' + ENIGMA2_PATH_LISTS + '*ncam*' + " " + camfolders + ' ; mv -f ' + ENIGMA2_PATH_LISTS + '*wicardd*' + " " + camfolders + ' ; chmod -R 644 ' + camfolders if wlan is False else ' ; mv -f ' + ENIGMA2_PATH_LISTS + '*oscam*' + " " + camfolders + ' ; mv -f ' + ENIGMA2_PATH_LISTS + '*ncam*' + " " + camfolders + ' ; mv -f ' + ENIGMA2_PATH_LISTS + '*wicardd*' + " " + camfolders + ' ; chmod -R 644 ' + camfolders + ' ; init 6'
				for cam in [x for x in listdir(self.backupdirectory) if x.startswith("oscam") or x.startswith("ncam") or x.startswith("wicardd")]:
					camfolder = join(self.backupdirectory, cam)
					if camfolder:
						eConsoleAppContainer().execute('rm -rf ' + camfolders + '*oscam*' + " " + camfolders + '*ncam*' + " " + camfolders + '*wicardd*')
	...
	def dobackupChannelsList(self, answer):
		try:
			...
			if answer:
				...
				for cam in [x for x in listdir(camfolders) if "oscam" in x or "ncam" in x or "wicardd" in x]:
					camfolder = join(camfolders, cam)
					if camfolder:
						eConsoleAppContainer().execute('cp -rf ' + camfolders + "/*" + ' ' + self.backupdirectory + '/')
