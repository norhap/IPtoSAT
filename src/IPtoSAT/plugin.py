	...
	def doinstallChannelsList(self, answer):
		try:
			...
			if answer:
				...
				for filestuxbox in [x for x in listdir(self.backupdirectory) if ".xml" in x]:
					...
					if backupfilestuxbox:
						for fileschannelslist in [x for x in listdir(FILES_TUXBOX) if ".xml" in x and "timezone.xml" not in x]:
							...
							if tuxboxfiles:
								remove(tuxboxfiles)
				for camspa in [x for x in listdir(self.backupdirectory) if "cam" in x]:
					camfolderspa = camspa
				...
				command = 'init 3 ; mount -a'  if BoxInfo.getItem("socfamily") != "hisi3798mv200" and not exists(str(self.backupdirectory) + '/zerotier-one') and BoxInfo.getItem("distro") != "openspa" else 'init 6'
				...
				eConsoleAppContainer().execute('init 4 ; sleep 5 ; ' + cmdinstall + rmcamfolderspa + 'cp -a ' + self.backupdirectory + '/* ' + ENIGMA2_PATH_LISTS + '; rm -f ' + ENIGMA2_PATH_LISTS + '*Camd* ; chmod 644 ' + ENIGMA2_PATH_LISTS + '*' + scriptfolder + ' ; chmod -R 755 ' + USR_SCRIPT + ' ; mv -f ' + ENIGMA2_PATH_LISTS + 'interfaces /etc/network/ ; mv -f ' + ENIGMA2_PATH_LISTS + 'shadow /etc/ ; chmod 400 /etc/shadow ; mv -f ' + ENIGMA2_PATH_LISTS + 'inadyn.conf /etc/ ; chmod 644 /etc/inadyn.conf ; mv -f ' + ENIGMA2_PATH_LISTS + '*wpa_supplicant.wlan* /etc/ ; chmod 600 /etc/*wpa_supplicant* ; mv -f ' + ENIGMA2_PATH_LISTS + 'auto.network /etc/ ; chmod 644 /etc/auto.network ; mv -f ' + ENIGMA2_PATH_LISTS + 'fstab /etc/ ; chmod 644 /etc/fstab ; mv -f ' + ENIGMA2_PATH_LISTS + 'CCcam.cfg /etc/ ; chmod 644 /etc/CCcam.cfg ; mv ' + ENIGMA2_PATH_LISTS + 'cables.xml ' + FILES_TUXBOX + '/ ; mv ' + ENIGMA2_PATH_LISTS + 'atsc.xml ' + FILES_TUXBOX + '/ ; mv ' + ENIGMA2_PATH_LISTS + 'terrestrial.xml ' + FILES_TUXBOX + '/ ; mv ' + ENIGMA2_PATH_LISTS + 'satellites.xml ' + FILES_TUXBOX + '/' + f'{dumped}')
		except Exception as err:
			self.session.open(MessageBox, "ERROR: %s" % str(err), MessageBox.TYPE_ERROR, default=False, timeout=10)
